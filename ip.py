...
# -*- coding: utf-8 -*-
# 📈 페이지 2 — IP 성과 자세히보기 (Standalone, gspread+ServiceAccount, no fallback, sections-based)
# 실행: streamlit run ip_detail_page_gspread_v2.py
#
# 필수 secrets.toml 예시
# [gcp_service_account]
...
st.set_page_config(
    page_title="IP 성과 자세히보기 — 단일 페이지 (gspread v2)",
    layout="wide",
...
def _get_service_account_from_secrets() -> dict:
    """
    secrets의 [gcp_service_account] 섹션(dict) 또는 루트 키(딕셔너리/문자열 JSON)를 수용.
    """
    info = None
    ...
    if not info:
        st.error("secrets.toml에 [gcp_service_account] 섹션이 필요합니다.")
        st.stop()

    # Streamlit secrets에서 개행이 \n 형태일 수 있으므로 보정
    if "private_key" in info and isinstance(info["private_key"], str):
        info["private_key"] = info["private_key"].replace("\\n", "\n")

    return info

def _get_sheet_ids_from_secrets() -> tuple[str, str]:
    sheets = st.secrets.get("sheets", {})
    if not isinstance(sheets, dict):
        st.error("secrets.toml에 [sheets] 섹션이 필요합니다.")
        st.stop()
    sid = str(sheets.get("SHEET_ID", "")).strip()
    gid = str(sheets.get("RAW_GID", "")).strip() or str(sheets.get("gid", "")).strip()
    if not sid or not gid:
        st.error("[sheets]에 SHEET_ID와 RAW_GID가 필요합니다.")
        st.stop()
    return sid, gid

@st.cache_data(ttl=600, show_spinner=True)
def load_raw_dataframe() -> pd.DataFrame:
    sa_info = _get_service_account_from_secrets()
    sheet_id, raw_gid = _get_sheet_ids_from_secrets()
    ...
    for w in sh.worksheets():
        if str(w.id) == str(raw_gid):
            ...
            break
    if ws is None:
        st.error(f"gid={raw_gid} 워크시트를 찾을 수 없습니다.")
        st.stop()
    ...
    seen = {}
    for h in header:
        name = (h or "").strip() or "Unnamed"
        cnt = seen.get(name, 0)
        cleaned.append(f"{name}.{cnt}" if cnt > 0 else name)
        seen[name] = cnt + 1
