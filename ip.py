# 📈 IP 성과 자세히보기 — Standalone v2.1 (no-floating + Pyramid restored)
# '플로팅(hover-lift)' 효과 전역 제거 + 성별×연령 인구 피라미드 복구
...
#region [ 6. 공통 집계 유틸: 데모 / 성별-연령 피라미드 ]
# =====================================================
...
def render_ip_detail(ip_selected: str, on_air_data: Dict[str, List[Dict[str, str]]]):
    ...
    with main_tab:
        ...
        # KPI: 앞/뒤 회차
        # 선택 주차의 앞 회차 / 뒷 회차
        def _week_to_front_back_eps_local(week: int) -> tuple[Optional[int], Optional[int]]:
            if week is None: return (None, None)
            front, back = 2*week - 1, 2*week
            eps = f.dropna(subset=["회차_num"])["회차_num"].astype(int).unique().tolist()
            return (front if front in eps else None, back if back in eps else None)

        ep_front, ep_back = _week_to_front_back_eps_local(selected_week)
        ...
        # === 맨 하단: 성별×연령 인구 피라미드 복구 ===
        st.divider()
        st.markdown("<div class='sec-title'>👥 시청자 데모 피라미드</div>", unsafe_allow_html=True)
        # 데모 데이터: metric == "시청인구" & '데모' 열 존재
        demo_src = f[(f["metric"] == "시청인구") & f["데모"].notna()].copy() if "데모" in f.columns else pd.DataFrame()
        render_gender_pyramid(st, "성별 × 연령대 분포 (해당 IP 전체)", demo_src, height=300)
